      ...
      <div class="mui-card-content">
        <div class="mui-card-content-inner">
          <p class="price">
            <span>
              市场价:
              <span class="last_price">{{this.shoppingInfo.last_price}}</span>
            </span>
            <span>
              销售价：
              <span class="now_price">{{this.shoppingInfo.now_price}}</span>
            </span>
          </p>
          <p class="buy_num">
            购买数量：
            <numbox @getcount="getSelcetedCount" :max="this.shoppingInfo.storageNum - this.shopNum"></numbox>
          </p>
...
export default {
  ...
  methods: {
    getShoppingInfo() {
      this.$http
        .get("nba/", {
          params: { key: "15632ad0534191c2eee477cf3de945e1", num: 47 }
        })
        .then(result => {
          if (result.body.code === 200) {
            this.runningImgList = result.body.newslist.slice(
              this.id,
              this.id + 3
            );
            ...
            console.log(this.shoppingInfo);
              ...
              this.shoppingInfo.last_price = parseInt(5499 + + this.id);
              this.shoppingInfo.now_price = parseInt(4399 + this.id);
            console.log(this.runningImgList[0]);
            console.log(this.shoppingInfo);
            if (this.runningImgList[0] === this.shoppingInfo) {
              console.log("5");
            }
            //问题1：为什么两者会相等；
            // 问题2：为什么渲染数据时用this.runningImgList[0]会报错，用this.shoppingInfo就不会报错;
            // 问题3：为什么第一次打印shoppingInfo时会把后边添加的属性打印出来
          }
        });
    },
    ...
    addShoppingcar() {
      if (this.shopNum + this.selectedCount <= this.shoppingInfo.storageNum) {
        this.ballFlag = !this.ballFlag;
        var shopInfo = {
          id: parseInt(this.id),
          count: this.selectedCount,
          price: this.shoppingInfo.now_price,
          selected: true
        };
        this.$store.commit("addToShopCar", shopInfo);
      } else {
        Toast("库存不足");
      }
